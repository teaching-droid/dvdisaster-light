...
`dvdisaster Light 0.2.0 (based on dvdisaster 0.79.10-pl6)`: Light 側の番号は本フォークのリリースを数え、ベース側の番号はコーデックの由来となった上流の正確な状態を示します。ファイル形式内のバージョン欄はベース版に固定されているため、他の dvdisaster バージョンもファイルを正しく解釈できます。
